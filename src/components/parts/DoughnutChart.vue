<template>
  <div class="doughnut_chart" style="position:relative;">
    <svg :width="width" :height="height" viewBox="0 0 200 200">
      <!-- Background circle -->
      <path :d="dBg" fill="transparent" :stroke="backgroundColor" :stroke-width="strokeWidth"></path>
      <!-- Move to start position, start drawing arc -->
      <path :d="d" fill="transparent" :stroke="foregroundColor" :stroke-width="strokeWidth"></path>
    </svg>
...
export default {
  ...
  computed: {
    ...
    visibleEmptyText() {
      return (
        (this.percent === "0" || this.percent === 0) && this.emptyText !== ""
      );
    }
  }
};
